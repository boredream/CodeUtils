        ...
        if (1 == 1) {
            ...
            for (String line : lines) {
                ...
                if (parts.length >= 4) {
                    ...
                    String storyId = parts[1];
                    String storyTitle = parts[2].replaceAll(" 待办$", ""); // 移除末尾的"待办"
                    String person = parts[3];
                    String personNumber = ShUser.getUserNumber(person); // 获取工号
                    String sp = parts[5];

                    // 构建新的行
                    result.append(taskName).append("\t")
                          .append(storyId).append("\t")
                          .append(storyTitle).append("\t")
