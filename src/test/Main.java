...
public class Main {
    ...
    public static void main(String[] args) throws Exception {
        ...
        if (date != null && time4Code != null) {
            ...
            JsonObject joDateLocation = date.get("location").getAsJsonObject();
            JsonObject joTime4CodeLocation = time4Code.get("location").getAsJsonObject();
            // TODO：规则待优化
            // 如果时间在日期下面；且二者的left相近，则确认命中
            int dateTop = joDateLocation.get("top").getAsInt();
            int dateHeight = joDateLocation.get("height").getAsInt();
            int dateLeft = joDateLocation.get("left").getAsInt();
            int time4codeTop = joTime4CodeLocation.get("top").getAsInt();
            int time4codeLeft = joTime4CodeLocation.get("left").getAsInt();
            // 因为倾斜所以添加一些容差
            ...
            if (dateTop + dateHeight - heightOffset < time4codeTop && Math.abs(dateLeft - time4codeLeft) < leftAlignOffset) {
                // 去掉时分，拼接成批次
                result = date.get("words").getAsString() + time4Code.get("words").getAsString().substring(5);
            }
        } else if (date4Code != null) {
            // 日期4字符：确认是袋装
            result = date4Code.get("words").getAsString();
        } else if (date != null && code4 != null) {
            // [低概率] 日期+4字符：可能是袋装。继续判断位置
            JsonObject joDateLocation = date.get("location").getAsJsonObject();
            JsonObject joCode4Location = code4.get("location").getAsJsonObject();
            // TODO：规则待优化
            // 如果4字符在日期的右边；且两者top相近，则确认命中
            int dateLeft = joDateLocation.get("left").getAsInt();
            int dateWidth = joDateLocation.get("width").getAsInt();
            int dateTop = joDateLocation.get("top").getAsInt();
            int time4codeLeft = joCode4Location.get("left").getAsInt();
            int time4codeTop = joCode4Location.get("top").getAsInt();
            // 因为倾斜所以添加一些容差
            ...
            if (Math.abs(dateTop - time4codeTop) < topAlignOffset && dateLeft + dateWidth < time4codeLeft + horSpaceOffset) {
                // 去掉时分，拼接成批次
                result = date.get("words").getAsString() + code4.get("words").getAsString();
            }
        }
