        ...
        for (String configInfo : FileUtils.readToStringLines(configFile)) {
            ...
            switch (kv[0].trim()) {
                ...
                case "tag_type":
                    try {
                        if (kv.length < 2) {
                            System.out.println("配置信息错误: tag_type 为必填项");
                            return null;
                        }

                        tag_type = Integer.parseInt(kv[1].trim());
                        if (tag_type < 1 || tag_type > 4) {
                            System.out.println("配置信息错误: tag_type 必须在1~4范围内");
                            return null;
                        }
                    } catch (Exception e) {
                ...
                case "price":
                    ...
                    try {
                        if (kv.length < 2) {
                            System.out.println("配置信息错误: 非免费时 price 为必填项");
                            return null;
                        }
                        ...
                        System.out.println("配置信息错误: age_range 必须为数字, 可以是2位以内的小数");
                        return null;
                    }
                    break;
